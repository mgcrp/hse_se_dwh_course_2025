...
10) Создадим новую таблицу
    ...
    - Содержание файла
```html
{{ config(materialized='table', schema='presentation', alias='gmv_by_store_category') }}
...
- relationships:
    ...
    field: id
```
    - Можно писать собственные тест-макросы `macros/custom_tests/test_positive_values.sql`
```
...
{% endtest %}
```
    - Можно писать точечные тесты `tests/test_orders_with_negative_gmv.sql`
```
SELECT *
FROM {{ ref('presentation__gmv_by_store_category') }}
WHERE sales_sum < 0
```
...
copy tpch.customer from  '/var/lib/postgresql/tpch_data/customer.csv' WITH (FORMAT csv, DELIMITER '|');
copy tpch.lineitem from  '/var/lib/postgresql/tpch_data/lineitem.csv' WITH (FORMAT csv, DELIMITER '|');
copy tpch.nation from  '/var/lib/postgresql/tpch_data/nation.csv' WITH (FORMAT csv, DELIMITER '|');
copy tpch.orders from  '/var/lib/postgresql/tpch_data/orders.csv' WITH (FORMAT csv, DELIMITER '|');
copy tpch.part from  '/var/lib/postgresql/tpch_data/part.csv' WITH (FORMAT csv, DELIMITER '|');
copy tpch.partsupp from  '/var/lib/postgresql/tpch_data/partsupp.csv' WITH (FORMAT csv, DELIMITER '|');
copy tpch.region from  '/var/lib/postgresql/tpch_data/region.csv' WITH (FORMAT csv, DELIMITER '|');
copy tpch.supplier from  '/var/lib/postgresql/tpch_data/supplier.csv' WITH (FORMAT csv, DELIMITER '|');
```
4)
```bash
dbt run -m tag:raw
